      ...
      <div className="min-h-screen bg-[#0f172a] flex items-center justify-center p-6 text-center">
        <div className="bg-white/10 backdrop-blur-2xl p-12 rounded-[2rem] border border-white/20 w-full max-w-md shadow-2xl animate-in zoom-in duration-300">
           ...
           <p className="text-blue-100/70 text-sm font-medium leading-relaxed mb-8">
             Vaša registracija je uspješno poslana. Super Admin će pregledati vaše podatke i odobriti pristup u najkraćem roku.
           </p>
           <button onClick={() => setMode('LOGIN')} className="w-full bg-blue-600 text-white font-black uppercase py-4 rounded-2xl">Povratak na prijavu</button>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-[#0f172a] flex items-center justify-center p-6 relative overflow-hidden">
      ...
      <div className="bg-white/10 backdrop-blur-2xl p-8 md:p-12 rounded-[2.5rem] shadow-2xl border border-white/20 w-full max-w-lg relative z-10 transition-all">
        <div className="text-center mb-10">
          ...
          <h2 className="text-3xl font-black text-white tracking-tighter uppercase">BH-ERP <span className="text-blue-500">Pro</span></h2>
          <p className="text-blue-200/40 text-[10px] font-black tracking-[0.2em] uppercase mt-2">{mode === 'LOGIN' ? 'Access Control' : 'Join Platform'}</p>
        </div>
        ...
        {mode === 'LOGIN' ? (
          <div className="space-y-6">
            ...
            >
              {loading ? 'Provjera...' : 'Pristupi sistemu'}
            </button>
            <p className="text-center text-white/40 text-xs font-medium">
              Nemate nalog? <button onClick={() => setMode('REGISTER')} className="text-blue-400 font-bold hover:underline">Registrujte firmu</button>
            </p>
          </div>
        ) : (
          <div className="space-y-6">
            ...
            {step === 1 ? (
              <div className="space-y-6 animate-in slide-in-from-right-4 duration-300">
                ...
                <button onClick={() => setStep(2)} disabled={!regData.email || !regData.password} className="w-full bg-blue-600 text-white font-black uppercase py-4 rounded-2xl disabled:opacity-50">Nastavi na podatke firme</button>
              </div>
            ) : (
              <div className="space-y-4 animate-in slide-in-from-right-4 duration-300">
                ...
                <div className="flex gap-4 pt-4">
                  <button onClick={() => setStep(1)} className="flex-1 border border-white/10 text-white font-black uppercase py-4 rounded-2xl text-xs">Nazad</button>
                  <button onClick={handleRegister} disabled={loading} className="flex-[2] bg-blue-600 text-white font-black uppercase py-4 rounded-2xl text-xs shadow-lg shadow-blue-900/40">Završi registraciju</button>
                </div>
              </div>
            )}
            <button onClick={() => { setMode('LOGIN'); setStep(1); }} className="w-full text-white/40 text-xs font-bold hover:text-white transition">Već imam nalog? Prijavi se</button>
          </div>
        )}
      </div>
    </div>
